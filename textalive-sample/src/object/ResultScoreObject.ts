...
export default class ResultScoreObject {
    ...
    public create(param: ResultScoreObjectCreateParam): void {
        this._resultNameImage = param.scene.add.image(param.posX, param.posY, param.resultNameKey);
        this._resultNameImage.setDepth(param.depth ? param.depth : 0).setOrigin(0.0, 0.5);

        this._underLineImage = param.scene.add.image(param.posX, param.posY + this._resultNameImage.height / 2 + 10, param.underlineKey);
        ...
        for (let i = 0; i < this._laneImages.length; i++) {
            ...
            image.setDepth(param.depth ? param.depth : 0).setOrigin(0.0, 0.5);
            ...
            text.setDepth(param.depth ? param.depth : 0).setOrigin(1.0, 0.5);
