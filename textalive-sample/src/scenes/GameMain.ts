...
let circleScale = 0;
let circleImg;
let circleSwitch = false;
const circleOffset = 0; // 円の中心を示す値

export default class GameMain extends Phaser.Scene {
  frameCount = 0;

  api: TextaliveApiManager;

  musicStart = false;

  firstLane: number = 120;
  secondLane: number = 320;
  thirdLane: number = 520;
  ...
  // ハートオブジェクト
  private laneHeartObjectArray: LaneHeartObject[];
  ...
  init(): void {
    ...
    const url = this.musicSelectScene.selectMusic[2];
  ...
  create(): void {
    ...
    // オブジェクトの生成
    // 背景
    const backImg = this.add.image(500, 350, 'backImg');
    backImg.alpha = 0.2;
    ...
    // スコアの設定
    const scoreT = this.add.text(30, 625, 'Score', { font: '18px Arial' });
    ...
    for (let j = 0; j < GameMain.LANE_SIZE; j++) {
      for (let i = 0; i < GameMain.AUDIENCE_SET_SIZE; i++) {
        this.audience[j][i].createAudience(
          this.add.image(880 - 110 * i, this.lanePosition[j], 'audience')
        );
      }
    }
    ...
    if (this.api.getPositionTime() != null && this.api.getPositionTime() != 0) {
      ...
      if (this.initFlag) {
        ...
        for (let i = 0; i < this.lyrics.length; i++) {
          this.lyricLine[i] = this.add.text(0, 636, this.lyrics[i].getText(), {
            font: '32px Arial',
          });
      ...
      const lyric = this.api.getCurrentLyric(time);
      let lyricText;
      let lyricIndex;
      if (lyric != null) {
  ...
  private updateLyricLine(currentLyricIndex) {
    ...
    if (
      ...
      typeof this.lyrics[currentLyricIndex] == 'undefined' ||
      currentLyricIndex <= this.lyricLineStartPos - 1
    ) {
      return;
    }
    // 直前に打ち出した歌詞データ
    const nextVisibleText = this.lyrics[currentLyricIndex];
  ...
  private setLaneColor(laneName: String, color: String) {
    ...
    switch (laneName) {
      case 'first':
        ...
        this.laneHeartObjectArray[0].image = this.add.image(
          this.heartX,
          this.firstLane,
          'heart_' + color
        );
        ...
        this.firstLaneLine = this.add.image(
          430,
          this.firstLane,
          'line_' + color
        );
      ...
      case 'second':
        ...
        this.laneHeartObjectArray[1].image = this.add.image(
          this.heartX,
          this.secondLane,
          'heart_' + color
        );
      ...
      case 'third':
        ...
        this.laneHeartObjectArray[2].image = this.add.image(
          this.heartX,
          this.thirdLane,
          'heart_' + color
        );
        ...
        this.thirdLaneLine = this.add.image(
          430,
          this.thirdLane,
          'line_' + color
        );
